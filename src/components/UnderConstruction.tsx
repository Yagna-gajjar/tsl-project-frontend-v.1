...
const ConstructionError = ({
  ...
  const themeClasses = {
    ...
    card: theme === "dark" ? "bg-gray-800 border border-gray-700" : "bg-white",
    title: theme === "dark" ? "text-gray-100" : "text-gray-800",
    primaryText: theme === "dark" ? "text-gray-200" : "text-gray-600",
    ...
    pulseRing: theme === "dark" ? "bg-orange-500" : "bg-orange-400",
  };

  return (
    <div
      className={`min-h-screen w-full ${themeClasses.background} flex items-center justify-center transition-colors duration-500 relative`}
    >
      <motion.div
      ...
      >
        <div
          className={`absolute top-0 left-0 right-0 h-2 ${themeClasses.topBorder}`}
        ></div>
        ...
        >
          <motion.div className="relative" animate={bounceAnimation}>
            <div
              className={`${themeClasses.iconBg} rounded-full p-4 shadow-lg`}
            >
        ...
        >
          ...
          <motion.p
            ...
            variants={slideInUp}
          >
            Our digital construction crew is working hard to fix this issue.
            Please try again or navigate back to safety.
          </motion.p>

          <motion.div className="space-y-3" variants={slideInUp}>
            {showRetry && (
              <motion.button
                onClick={onRetry}
