	...
	return (
		...
		>
			...
			<div className="px-4 py-3 border-b border-slate-100 dark:border-zinc-800 flex justify-between items-center bg-slate-50/50 dark:bg-zinc-900/50">
				<h3 className="font-semibold text-slate-800 dark:text-slate-200">{group.category}</h3>
				<Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={onCreateItem}>
			...
			<div className="divide-y divide-slate-100 dark:divide-zinc-800 max-h-[350px] overflow-y-auto">
				...
				{items.length === 0 && (
					<div className="p-8 text-center text-sm text-muted-foreground">Empty Category</div>
				)}
			</div>
		</motion.div>
	);
