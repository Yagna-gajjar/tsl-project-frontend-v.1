...
export default function MembershipExcelUpload({
  ...
  const handleValidateRow = useCallback((row: MembershipImportRow) => {
    ...
    if (isNaN(Number(row.membershipMasterId)))
      return "Master ID must be a valid number";
    if (isNaN(Number(row.accountId)))
    ...
    async (row: MembershipImportRow) => {
      const payload = {
        ...
        graceDate: row.graceDate
          ? new Date(row.graceDate).toISOString()
          : new Date().toISOString(),
        cancelationDate: row.cancelationDate
          ? new Date(row.cancelationDate).toISOString()
          : null,
        ...
        caDepositPRRequiredFBalance: Number(
          row.caDepositPRRequiredFBalance || 0
        ),
        caDepositPRRequiredCBalance: Number(
          row.caDepositPRRequiredCBalance || 0
        ),
        ...
        qualifyingRecieptNo: row.qualifyingRecieptNo
          ? Number(row.qualifyingRecieptNo)
          : null,
      };
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-950 z-10">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                  Link active accounts to membership plans and financial
                  balances.
                </p>
              </div>
              <button
