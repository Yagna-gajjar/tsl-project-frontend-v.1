...
import { Search, X, ChevronDown, ChevronUp, Users, ArrowRight, ArrowLeft, CheckCircle2, Layers } from "lucide-react";
...
export default function AccountMemberBulkFormModal({ isOpen, onClose }: Props) {
	...
	const [searchParams, setSearchParams] = useState<MemberSearchParams>({
		name: "", address: "", contactNumber: "", ageGroup: "",
		personalStatus: "", idProofNumber: "", includeCasual: false,
	});
	...
	const handleSearchChange = (field: keyof MemberSearchParams, value: string | boolean) => {
	...
	const updateSelectedMember = (id: number, field: keyof SelectedMemberConfig, value: any) => {
		setSelectedMembers(prev => ({
			...prev,
	...
	const applyBulkRelationship = () => {
		...
		toast({ title: "Applied", description: `Relationship set to '${bulkRel}' for all users.` });
	};
	...
	const applyBulkBilling = (val: boolean) => {
		...
		toast({ title: "Applied", description: `Billing ${val ? 'enabled' : 'disabled'} for all users.` });
	};
	...
	return (
		<Dialog open={isOpen} onOpenChange={(o) => !o && onClose()}>
			<DialogContent className="max-w-[1200px] p-0 gap-0 bg-background overflow-hidden h-[90vh] flex flex-col rounded-xl border shadow-xl">
				...
				<FormHeader
					title={viewMode === 'search' ? "Step 1: Select Members" : "Step 2: Configure & Link"}
					onClose={onClose}
				/>
					...
					<div className="flex flex-col h-full overflow-hidden animate-in fade-in zoom-in-95 duration-200">
						<div className="p-4 border-b bg-muted/5 space-y-4">
							<div className="max-w-md">
								<Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Target Account</Label>
							...
							<div className="flex flex-col gap-3">
								...
								{showFilters && (
									<div className="grid grid-cols-2 md:grid-cols-4 gap-2 pt-1 p-2 bg-muted/20 rounded-md border border-border/40">
										<Input placeholder="Address / City" className="h-8 text-xs bg-background" value={searchParams.address} onChange={(e) => handleSearchChange('address', e.target.value)} />
										<Input placeholder="Mobile" className="h-8 text-xs bg-background" value={searchParams.contactNumber} onChange={(e) => handleSearchChange('contactNumber', e.target.value)} />
										<Input placeholder="ID Proof" className="h-8 text-xs bg-background" value={searchParams.idProofNumber} onChange={(e) => handleSearchChange('idProofNumber', e.target.value)} />
										<div className="flex items-center justify-center border rounded-md h-8 bg-background px-2">
											<Checkbox id="casual" checked={searchParams.includeCasual} onCheckedChange={(c) => handleSearchChange('includeCasual', c as boolean)} />
											<Label htmlFor="casual" className="ml-2 text-[10px] cursor-pointer">Include Casual</Label>
										</div>
									</div>
								)}
							</div>
						...
						>
							{isLoadingMembers && visibleCount === 50 ? (
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
									{[...Array(9)].map((_, i) => <div key={i} className="h-24 bg-muted/20 animate-pulse rounded-lg border" />)}
								</div>
											...
											>
												<div className="flex justify-between items-start mb-2">
													...
													{member.personalStatus && (
														<Badge variant="secondary" className="text-[10px] px-1.5 h-5 font-normal">{member.personalStatus}</Badge>
													)}
												</div>

												<div className="text-xs text-muted-foreground space-y-1 ml-6">
													<div className="truncate">{member.contactNumber || "No Contact"}</div>
													<div className="truncate opacity-80">{member.address || member.city || "No Address"}</div>
												</div>
											</div>
										)
									})}
						...
						<div className="p-4 border-t bg-background flex justify-between items-center z-10 shadow-[0_-5px_10px_rgba(0,0,0,0.03)]">
							<div className="text-sm text-muted-foreground">
								<span className="font-semibold text-primary">{selectedCount}</span> members selected
							</div>
							<Button
					...
					<div className="flex flex-col h-full overflow-hidden animate-in slide-in-from-right-10 duration-300">
						...
						<div className="p-4 border-b bg-muted/10 space-y-3">
							<div className="flex items-center gap-2 mb-2">
								<Button variant="ghost" size="sm" onClick={() => setViewMode('search')} className="h-7 px-2 -ml-2 text-muted-foreground">
							...
							<div className="flex flex-col md:flex-row gap-4 items-end md:items-center bg-background p-3 rounded-lg border shadow-sm">
								<div className="flex-1 w-full flex items-center gap-2">
									...
									<Input
										placeholder="Set Relationship for ALL..."
										value={bulkRel}
										onChange={(e) => setBulkRel(e.target.value)}
										className="h-8 text-sm"
									/>
									<Button size="sm" variant="secondary" onClick={applyBulkRelationship} disabled={!bulkRel} className="h-8 whitespace-nowrap">
										Apply All
									</Button>
								</div>
								...
								<div className="flex items-center gap-3">
									<span className="text-sm text-muted-foreground whitespace-nowrap">Billing for ALL:</span>
						...
						<div className="grid grid-cols-12 gap-4 px-6 py-2 bg-muted/5 border-b text-xs font-semibold text-muted-foreground uppercase tracking-wider">
							...
							<div className="col-span-12 md:col-span-2 text-right md:text-center mt-1 md:mt-0">Billing</div>
						</div>
						...
						<div className="flex-1 overflow-y-auto bg-background">
                            ...
                            `}
								>
									<div className="col-span-4 md:col-span-3 font-medium text-sm truncate flex items-center gap-2">
										...
										<span className="truncate">{item.memberData.memberFirstName} {item.memberData.memberLastName}</span>
									</div>

									<div className="col-span-4 md:col-span-3 text-xs text-muted-foreground truncate">
										{item.memberData.contactNumber || "-"}
										<span className="block opacity-70 text-[10px]">{item.memberData.personalStatus}</span>
									</div>
									...
									<div className="col-span-4 md:col-span-4">
										<Input
											...
											onChange={(e) => updateSelectedMember(item.memberData.memberId, 'relationship', e.target.value)}
										/>
									</div>
                                    ...
                                    `}
											onClick={() => updateSelectedMember(item.memberData.memberId, 'linkBilling', !item.linkBilling)}
										>
											<span className="text-[10px] font-semibold">{item.linkBilling ? "Linked" : "Unlinked"}</span>
											<Checkbox checked={item.linkBilling} className="h-3.5 w-3.5 pointer-events-none data-[state=checked]:bg-green-600 data-[state=checked]:border-green-600" />
										</div>
									</div>
								</div>
							))}
							...
							{selectedList.length === 0 && (
								<div className="flex flex-col items-center justify-center h-40 text-muted-foreground">
									...
									<Button variant="link" onClick={() => setViewMode('search')}>Go back to select members</Button>
								</div>
							)}
						</div>
