			...
			<header className="flex-none px-6 py-5 border-b border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 z-20 shadow-sm">
				<div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
					<div>
						...
						<div className="flex items-center gap-2 mt-1">
							<p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
								{billingData?.data[0]?.academyName || (academy ? `Academy #${academy.academyId}` : "Select Academy")}
							</p>
							{academy?.academyType && (
					...
					<div className="flex items-center gap-3 bg-slate-100 dark:bg-neutral-800 p-1.5 rounded-xl border border-slate-200 dark:border-neutral-700 shadow-inner">
						...
						<span className="text-slate-400 dark:text-slate-600 font-medium">to</span>
						<div className="relative group">
							<input
								type="date"
								value={endDate}
			...
			<main className="flex-1 overflow-y-auto p-4 md:p-8 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-neutral-700">
				<div className="max-w-7xl mx-auto space-y-8">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						>
							<div>
								<p className="text-slate-500 dark:text-slate-400 text-sm font-medium uppercase tracking-wider">Gross Revenue</p>
						...
						>
							<div>
								<p className="text-slate-500 dark:text-slate-400 text-sm font-medium uppercase tracking-wider">Total Deductions</p>
								<h2 className="text-4xl font-bold text-rose-600 dark:text-rose-500 mt-1">
									{loading ? "..." : `-${formatCurrency(financials.totalDeductions)}`}
								</h2>
							</div>
					...
					{/* --- REVENUE DISTRIBUTION SECTION (The Core Logic) --- */}
					<motion.div
					...
					>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
							{/* 1. Main Share Card (Takes the deduction hit) */}
							<ShareCard
								title="Academy Main"
					...
					{/* --- DEBIT NOTES TABLE (Detailing the deductions) --- */}
					{debitNotes.length > 0 && (
							...
							<div className="overflow-x-auto">
								<table className="w-full text-left text-sm">
									...
									<tbody className="divide-y divide-rose-200/50 dark:divide-rose-900/20">
										{debitNotes.map((note) => (
											<tr key={note.debitNoteId} className="hover:bg-rose-100/50 dark:hover:bg-rose-900/20 transition-colors">
												<td className="px-6 py-4 font-mono text-rose-900 dark:text-rose-100">
													{formatDateDisplay(note.debitNoteDate)}
												</td>
										...
										{billingData.data.map((bill) => (
											<React.Fragment key={bill.enrollmentId}>
												{bill.monthSlices.map((slice, sliceIndex) => (
													<tr key={`${bill.enrollmentId}-${sliceIndex}`} className="hover:bg-slate-50 dark:hover:bg-neutral-800/50 transition-colors">
														<td className="px-6 py-4 font-medium text-slate-900 dark:text-slate-100">
															{bill.memberName}
															<div className="text-xs text-slate-500 font-normal">{bill.courseName}</div>
														</td>
														<td className="px-6 py-4 text-slate-600 dark:text-slate-400 text-xs">
															{formatDateDisplay(slice.fromDate)} - {formatDateDisplay(slice.toDate)}
														</td>
														<td className="px-6 py-4 text-right text-slate-600 dark:text-slate-400">{slice.days}</td>
														<td className="px-6 py-4 text-right text-slate-500">{formatCurrency(slice.rate)}</td>
